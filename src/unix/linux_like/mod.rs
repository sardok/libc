...
s! {
    ...
    pub struct sched_param {
        ...
        #[cfg(any(target_env = "fortanixvme", target_env = "musl", target_os = "emscripten", target_env = "ohos"))]
...
cfg_if! {
    if #[cfg(all(
        target_os = "linux",
        any(target_env = "fortanixvme", target_env = "gnu", target_env = "musl", target_env = "ohos")
    ))] {
        // WARN: deprecated
...
cfg_if! {
    if #[cfg(not(any(target_env = "musl", target_env = "fortanixvme", target_os = "emscripten")))] {
        extern "C" {
